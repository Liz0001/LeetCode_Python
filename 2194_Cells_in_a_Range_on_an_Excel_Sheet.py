"""
2194. Cells in a Range on an Excel Sheet from Leetcode.
...
<row> is the row number r of the cell.
The rth row is represented by the integer r.
...
The cells should be represented as strings in the format mentioned
...
class Solution:
    """Cells in a Range on an Excel Sheet Class."""

    def cellsInRange(self, s: str) -> List[str]:
        """Cells in a Range on an Excel Sheet."""
        alphabet = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
        cells = []
        for j in range(alphabet.find(s[0]), alphabet.find(s[3])+1):
            for i in range(int(s[1]), int(s[4])+1):
                cells.append(alphabet[j] + str(i))
        return cells
